...
| `{$URBACKUP_STUCK_FREEZE}` | 1 | Durée (en heures) pendant laquelle la progression du backup doit être figée pour déclencher les alertes "sauvegarde bloquée". Défaut : 1h. Augmenter à 2 ou 3 si des backups très lents génèrent des faux positifs. |
| `{$URBACKUP_STUCK_HIGH_DAYS}` | 5 | Nombre de jours continus de backup en cours avant alerte HIGH "sauvegarde bloquée". Combiné avec la progression figée depuis {$URBACKUP_STUCK_FREEZE}h. Défaut : 5 jours. Au-delà, intervention manuelle requise. |
| `{$URBACKUP_STUCK_WARN_DAYS}` | 3 | Nombre de jours continus de backup en cours avant alerte AVERAGE "sauvegarde possiblement bloquée". Combiné avec la progression figée depuis {$URBACKUP_STUCK_FREEZE}h. Défaut : 3 jours. Si le trigger HIGH se déclenche ({$URBACKUP_STUCK_HIGH_DAYS}j), intervenir. |
